...
VALIDATE_TARGET_REPO='https://harbor.taco-cat.xyz/tks'
EXCEPTION_LIST=
...
do
  ...
  docker run --rm -i -v $(pwd)/:/tmp --name image-checker ${DOCKER_IMAGE_REPO}/sktcloud/decapod-render:v3.1.0 helm2yaml/check_repo.py -m /tmp/$hr_file -r $VALIDATE_TARGET_REPO -t image $EXCEPTION_ARGS
